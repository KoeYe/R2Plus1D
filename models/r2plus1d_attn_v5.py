...
class TemporalSelfAttention(nn.Module):
    def __init__(self,
        ...
        self.ffn = nn.Sequential(
            ...
            nn.Linear(inner_dim, inner_dim*4, bias=True),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(inner_dim*4, inner_dim, bias=True),
            nn.Dropout(dropout),
        )
        ...
        self.mid_norm = nn.LayerNorm(inner_dim)
    ...
    def forward(self, x):
        ...
        attn_out, attn_w = self.attn(              # <<< keep weights
            q, k, v,
            # attn_mask=causal_mask,
            need_weights=True,              # return weights
        ...
        if self.training and torch.rand(1).item() < 0.001:   # ≈0.1 %
            ratio = attn_out.norm().div(x_seq.norm().add(1e-8)).item()
            # choose one:
            B_, Hh, TT, _ = attn_w.shape
            diag = torch.eye(TT, dtype=torch.bool, device=attn_w.device)\
                        .unsqueeze(0).unsqueeze(0)           # (1,1,T,T)
            rho = attn_w.masked_select(~diag).sum().div(attn_w.sum()).item()
            
            p = attn_w.transpose(1,2).reshape(-1, attn_w.size(-1))
            eps = 1e-8
            eye = torch.eye(p.size(1), device=p.device, dtype=torch.bool)
            H = -(p * (p + eps).log()).sum(-1).mean().item()
            #tau = p.topk(3, dim=-1).values.sum(-1).div(p.sum(-1) + eps).mean().item()

            print(f"[SA DEBUG] T:{T} lnT:{np.log(T)}  ||A||/||X||={ratio:5.3f}   off‑diag mass ρ={rho:5.3f}, H={H:4.2f}")
...
class R2Plus1D(nn.Module):
    def __init__(self, block, layers, num_classes=400, in_channels=3, use_mha=True, attn_heads=8):
        ...
        # print(f"layer1")
        self.layer1 = self._make_layers(block, 64, layers[0], init_stride=1, use_mha=True, attn_heads=attn_heads)
        # print(f"layer2")
        self.layer2 = self._make_layers(block, 128, layers[1], init_stride=2, use_mha=True, attn_heads=attn_heads)
        # print(f"layer3")
        self.layer3 = self._make_layers(block, 256, layers[2], init_stride=2, use_mha=True, attn_heads=attn_heads)
        # print(f"layer4")
        self.layer4 = self._make_layers(block, 512, layers[3], init_stride=2, use_mha=False, attn_heads=attn_heads)
