...
class TemporalSelfAttention(nn.Module):
    def __init__(self, inout_channels: int, num_heads: int = 1, dropout: float = 0.0):
        super().__init__()
        # self.channels   = in_channels
        self.channels = inout_channels // 16 * 2 # make sure it is even number
        # print(self.channels)

        self.proj_in = nn.Linear(inout_channels, self.channels)
        ...
        self.proj_out = nn.Linear(self.channels, inout_channels)
        self.dropout = nn.Dropout(dropout)
        self.norm_out = nn.LayerNorm(inout_channels)
    ...
    def forward(self, x):
        ...
        v = x_flat  # values untouched

        attn_mask = torch.triu(
            torch.ones(T, T, dtype=torch.bool, device=x.device), diagonal=1
        )

        attn_out, _ = self.attn(q_rot, k_rot, v, attn_mask=attn_mask)
        attn_out = self.dropout(self.proj_out(attn_out))
        # print(residual.shape)
        # print(attn_out.shape)
        attn_out = self.norm_out(attn_out + residual)
